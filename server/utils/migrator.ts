...
export const addTfIdfVector = (
    docs: DataDoc[]
): { [key in SparseVectorID]: Term } => {
    ...
    for (const doc of docs) {
        doc._source['image_path'] = path.win32.basename(
            doc._source['Image path']
        );
        ...
        const tf_vector = Object.fromEntries(
            Object.entries(freqs).map(([term, freq]) => [
                term,
                freq / docTerms.length,
            ])
        );
    ...
    const termToIdf = Object.fromEntries(
        Object.entries(termToDocSet).map(([term, setOfDocs]) => [
            term,
            Math.log10(numOfDocs / setOfDocs.size),
        ])
    );
    // create dictionary
    ...
    // ["a", "b", "c"] => {"0": "a", "1": "b", "2": "c"}
    const termToId = Object.fromEntries(
        sortedTerms.map((term, i) => [term, '' + i])
    );
    ...
    for (const doc of docs) {
        ...
        const sparseVecTfIdf = Object.fromEntries(
            Object.entries(termToTfIdf).map(([term, tfIdf]) => [
                termToId[term],
                tfIdf,
            ])
        );
...
export const makeIdToTerm = (
    dictionaryDocs: DictionaryDoc[]
): { [key in SparseVectorID]: Term } =>
